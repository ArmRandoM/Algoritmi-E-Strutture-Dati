...
public :     
	...
	Nodo<T>* cerca(T i) const {
        Nodo<T>* tmp = testa;        
        while(tmp != 0) { //iterazione fino a quando ci sono valori
            if(tmp->getValore() == i)  //definizione dell'operatore di uguaglianza in T
            	break;
            tmp = tmp->getNextNodo();
        }           
        return tmp;//se arriva qui ritornerà sempre tmp=0
    };
